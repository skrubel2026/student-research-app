...
def show_new_application_form():
    ...
    with st.form("application_form", clear_on_submit=True):
        ...
        with col_target_year:
            target_year_options = [this_year, this_year + 1, this_year + 2, this_year + 3, "➕ Type a new value"]
            target_year_choice = st.selectbox("Target Year *", target_year_options)
            if target_year_choice == "➕ Type a new value":
                target_year_choice = st.text_input("Enter target year")
...
def show_edit_application_form():
    ...
    with st.form("edit_form"):
        ...
        with col_target_year:
            ...
            target_year_options = [this_year, this_year + 1, this_year + 2, this_year + 3, "➕ Type a new value"]
            default_target_year_idx = (
                target_year_options.index(existing_target_year)
                if existing_target_year in target_year_options else len(target_year_options) - 1
            )
            target_year_choice = st.selectbox(
                "Target Year *", target_year_options,
                index=default_target_year_idx,
                key="edit_target_year",
            )
            if target_year_choice == "➕ Type a new value":
                target_year_choice = st.text_input(
                    "Enter target year", value=str(existing_target_year), key="edit_target_year_custom"
                )
        target_semester = f"{target_term} {target_year_choice}"
